...
var DEFAULT_SETTINGS_VALUES = {
  ...
  wrapGroupLabels: true,
  // Added by an LLM agent
  hideGroupLabels: false,
  // Added by an LLM agent
  boxedNodeLabels: true,
  // Added by an LLM agent
  defaultGroupLabelSize: "1",
  // Added by an LLM agent
  defaultGroupOpacity: "default",
  // Added by an LLM agent
  defaultEdgeWidth: "1",
  // Added by an LLM agent
  defaultArrowSize: "1",
  ...
  canvasFilterFeatureEnabled: true,
  // Added by an LLM agent
  ...
  autoOpenSingleNodePortals: true,
  // Added by an LLM agent
  ...
  edgeLabelMarkdownEnabled: false,
  // Added by an LLM agent
  edgeLabelMarkdownRenderMode: "embeds-only",
  // Added by an LLM agent
  edgeLabelMarkdownMaxWidth: 400,
  // Added by an LLM agent
  edgeLabelMarkdownMaxHeight: 300
  // Added by an LLM agent
};
var SETTINGS = {
  general: {
  ...
  edgeLabelMarkdownEnabled: {
    ...
    children: {
      ...
      edgeLabelMarkdownMaxWidth: {
        ...
        type: "number",
        parse: (value) => Math.max(50, parseInt(value) || 400)
      },
      edgeLabelMarkdownMaxHeight: {
        ...
        type: "number",
        parse: (value) => Math.max(50, parseInt(value) || 300)
      }
    }
  },
...
var CanvasPatcher = class extends Patcher {
  ...
  patchEdge(edge) {
    ...
    Patcher.patch(this.plugin, edge, {
      ...
      updatePath: Patcher.OverrideExisting((next) => function(...args) {
        const result = next.call(this, ...args);
...
var NO_RATIO = "No ratio enforcement";
var RATIO_OPTIONS = [
...
var NodeRatioCanvasExtension = class extends CanvasExtension {
  ...
  onNodeMenu(menu, node) {
    ...
    menu.addItem((item) => {
      item.setTitle("Set Aspect Ratio").setIcon("ratio").onClick(async () => {
        const newRatioString = await new AbstractSelectionModal(this.plugin.app, "Enter aspect ratio (width:height)", RATIO_OPTIONS.map((option) => option.label)).awaitInput();
  ...
  onPopupMenuCreated(canvas) {
    ...
    const currentRatio = (_a = selectedNode.getData().ratio) != null ? _a : null;
    ...
    menuOption.addEventListener("click", () => {
      var _a2, _b;
      const submenu = (_a2 = menuOption.parentElement) == null ? void 0 : _a2.querySelector(`#${menuOption.id}-submenu`);
      if (!submenu) return;
      const activeIndex = RATIO_OPTIONS.findIndex(
        (option) => option.ratio === null ? currentRatio === null : Math.abs(option.ratio - (currentRatio != null ? currentRatio : 0)) < 1e-3
      );
      if (activeIndex >= 0) (_b = submenu.children[activeIndex]) == null ? void 0 : _b.classList.add("is-active");
    });
    CanvasHelper.addPopupMenuOption(canvas, menuOption);
  }
  setNodeRatio(node, ratio) {
    const nodeData = node.getData();
    if (ratio === null) {
      node.setData({
        ...nodeData,
...
// src/canvas-extensions/group-label-wrap-canvas-extension.ts
var LABEL_WIDTH_PROPERTY = "--wrapped-group-label-width";
var GroupLabelWrapCanvasExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    /** The last `--zoom-multiplier` a canvas' labels were measured at */
    this.lastZoomMultipliers = /* @__PURE__ */ new WeakMap();
    this.zoomObservers = /* @__PURE__ */ new Map();
  }
  isEnabled() {
    return "wrapGroupLabels";
  }
  init() {
    this.plugin.register(() => this.stopObservingZoom());
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-changed",
      (canvas) => this.observeZoom(canvas)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-view-unloaded:before",
      (view) => this.stopObservingZoom(view.canvas)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:node-rendered",
      (_canvas, node) => this.updateLabelWidth(node)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:node-changed",
      (_canvas, node) => this.updateLabelWidth(node)
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (_canvas, node, _isEditing) => this.updateLabelWidth(node)
    ));
  }
  /**
  ...
  init() {
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (_canvas) => true,
        (_canvas) => this.toggleGroupLabelVisibility()
      )
    });
  }
...
// src/canvas-extensions/default-style-sizes-canvas-extension.ts
var DEFAULT_SIZE_SETTINGS = [
...
var DefaultStyleSizesCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-changed",
      (canvas) => this.updateDefaultSizes(canvas)
    ));
  }
  updateDefaultSizes(canvas) {
    const wrapperEl = canvas.wrapperEl;
    if (!wrapperEl) return;
    for (const { setting, datasetKey, unchangedValue } of DEFAULT_SIZE_SETTINGS) {
      const value = this.plugin.settings.getSetting(setting);
      if (value === (unchangedValue != null ? unchangedValue : UNCHANGED_SIZE)) delete wrapperEl.dataset[datasetKey];
...
var INVISIBLE_STYLE_VALUE = "invisible";
var INVISIBLE_STYLE_KEYS = {
...
var CommandsCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (canvas) => !canvas.readonly && this.getInvisibleStyleTargets(canvas).length > 0,
        (canvas) => this.toggleInvisibleStyle(canvas)
      )
    });
    this.plugin.addCommand({
      id: "create-text-node",
   ...
   */
  getInvisibleStyleTargets(canvas) {
    const targets = [];
    for (const element of canvas.selection) {
      const kind = element.path !== void 0 ? "edge" : "node";
   ...
   */
  toggleInvisibleStyle(canvas) {
    var _a, _b, _c;
    const targets = this.getInvisibleStyleTargets(canvas);
    if (targets.length === 0) return;
    const hide = !targets.every(({ element, keys }) => {
      var _a2;
      return ((_a2 = element.getData().styleAttributes) == null ? void 0 : _a2[keys.style]) === INVISIBLE_STYLE_VALUE;
    });
    for (const { element, keys } of targets) {
      const elementData = element.getData();
      const styleAttributes = (_a = elementData.styleAttributes) != null ? _a : {};
      if (styleAttributes[keys.style] === INVISIBLE_STYLE_VALUE === hide) continue;
      element.setData({
        ...elementData,
        styleAttributes: {
          ...styleAttributes,
          [keys.style]: hide ? INVISIBLE_STYLE_VALUE : (_b = styleAttributes[keys.restore]) != null ? _b : null,
          [keys.restore]: hide ? (_c = styleAttributes[keys.style]) != null ? _c : null : null
        }
      });
    }
    canvas.pushHistory(canvas.getData());
...
var PortalsCanvasExtension = class _PortalsCanvasExtension extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:node-created",
      (canvas, node) => void this.tryAutoOpenSingleNodePortal(canvas, node)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:node-removed",
  ...
  async tryAutoOpenSingleNodePortal(canvas, node) {
    var _a, _b;
    ...
    let portalFileData;
    try {
      portalFileData = JSON.parse(portalFileDataString);
    } catch (error) {
      return;
    }
    if (!(portalFileData == null ? void 0 : portalFileData.nodes) || portalFileData.nodes.length !== 1) return;
    const currentNode = canvas.nodes.get(node.id);
    if (!currentNode) return;
    const currentNodeData = currentNode.getData();
    if (currentNodeData.portal) return;
...
var CanvasFilterCanvasExtension = class _CanvasFilterCanvasExtension extends CanvasExtension {
  isEnabled() {
    return "canvasFilterFeatureEnabled";
  }
  init() {
    this.plugin.addCommand({
      id: "reset-filter",
    ...
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (canvas) => canvas.selection.size > 0,
        (canvas) => this.filterByColor(canvas)
      )
    });
    this.plugin.addCommand({
      id: "filter-by-tag",
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (_canvas) => true,
        (canvas) => void this.filterByTag(canvas)
      )
    });
    this.plugin.addCommand({
      id: "hide-selection",
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (canvas) => canvas.selection.size > 0,
        (canvas) => this.hideSelection(canvas, false)
      )
    });
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (canvas) => canvas.selection.size > 0,
        (canvas) => this.hideSelection(canvas, true)
      )
    });
    for (const connectionFilter of CONNECTION_FILTERS) {
      this.plugin.addCommand({
        ...
        checkCallback: CanvasHelper.canvasCommand(
          this.plugin,
          (canvas) => canvas.selection.size > 0,
          (canvas) => this.filterByConnections(canvas, connectionFilter)
        )
      });
    }
  }
  ...
  setElementsVisibility(elements, visible) {
    for (const element of elements) element == null ? void 0 : element.classList.toggle(FILTERED_OUT_CLASS, !visible);
  }
  setNodeVisibility(node, visible) {
    this.setElementsVisibility([node.nodeEl], visible);
  }
  setEdgeVisibility(edge, visible) {
    var _a;
    this.setElementsVisibility([edge.lineGroupEl, edge.lineEndGroupEl, (_a = edge.labelElement) == null ? void 0 : _a.wrapperEl], visible);
  }
  ...
  showOnlyNodes(canvas, nodeIdsToShow) {
    for (const node of canvas.nodes.values())
      this.setNodeVisibility(node, nodeIdsToShow === void 0 || nodeIdsToShow.has(node.id));
  }
  /** Shows the edges with the given ids and hides all others. Shows every edge if no ids are given. */
  showOnlyEdges(canvas, edgeIdsToShow) {
    for (const edge of canvas.edges.values())
      this.setEdgeVisibility(edge, edgeIdsToShow === void 0 || edgeIdsToShow.has(edge.id));
  }
  ...
  getGroupsContaining(allNodes, containedNodes) {
    return allNodes.filter(
      (node) => node.type === "group" && containedNodes.some((containedNode) => _CanvasFilterCanvasExtension.bboxContains(node, containedNode))
    );
  }
  // Filters
  filterByColor(canvas) {
    const colorsToShow = new Set([...canvas.selection].map((element) => {
      var _a;
  ...
  async filterByTag(canvas) {
    ...
    const nodesToShow = canvas.getData().nodes.filter((node) => this.getTagsOfNode(canvas, node).some((tag) => _CanvasFilterCanvasExtension.tagMatches(tag, selectedTag)));
    this.showOnlyNodesAndTheirEdges(canvas, nodesToShow);
  }
  ...
  hideSelection(canvas, includeConnectedEdges) {
  ...
  getAvailableTags(canvas) {
    var _a;
    const vaultTags = Object.keys((_a = this.plugin.app.metadataCache.getTags()) != null ? _a : {});
    const canvasTags = canvas.getData().nodes.flatMap((node) => this.getTagsOfNode(canvas, node));
    return [.../* @__PURE__ */ new Set([...vaultTags, ...canvasTags])].sort((a, b) => a.localeCompare(b));
  }
  getTagsOfNode(canvas, nodeData) {
    var _a, _b, _c, _d;
  ...
  static tagMatches(tag, filterTag) {
...
var SYNC_FENCE_PATTERN = /^(`{3,}|~{3,})sync[ \t]*\n([\s\S]*?)\n?\1[ \t]*$/;
function isEmbedOnlyLabel(source) {
  const trimmed = source.trim();
  if (trimmed.length === 0) return false;
  const fenceMatch = SYNC_FENCE_PATTERN.exec(trimmed);
  return isEmbedLines(fenceMatch ? fenceMatch[2] : trimmed);
}
function isEmbedLines(content) {
  const lines = content.split("\n").map((line) => line.trim()).filter((line) => line.length > 0);
  return lines.length > 0 && lines.every((line) => EMBED_LINE_PATTERN.test(line));
}

// src/canvas-extensions/edge-label-markdown-canvas-extension.ts
var RENDERED_CLASS = "ac-markdown-label";
var BLOCK_CONTENT_CLASS = "ac-markdown-label-block";
var BLOCK_CONTENT_SELECTOR = '.internal-embed, .external-embed, [class*="block-language-"], table, img, iframe, video, canvas, .callout';
var MAX_WIDTH_PROPERTY = "--ac-markdown-label-max-width";
var MAX_HEIGHT_PROPERTY = "--ac-markdown-label-max-height";
var INTERACTIVE_SELECTOR = "a, .internal-embed, .external-embed, .task-list-item-checkbox, button, input, .callout-fold, .copy-code-button";
var EdgeLabelMarkdownCanvasExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    /** The label source that is currently rendered, per label - guards against redundant re-renders */
    this.renderedSources = /* @__PURE__ */ new WeakMap();
    /** Owns the lifecycle of everything `MarkdownRenderer` created for a label (embeds, code blocks, ...) */
    this.renderComponents = /* @__PURE__ */ new WeakMap();
    /** The capture-phase click interceptor installed on a label wrapper */
    this.clickInterceptors = /* @__PURE__ */ new WeakMap();
    /** The core label class isn't exported, so its prototype gets patched lazily via the first instance */
    this.labelPrototypePatched = false;
  }
  isEnabled() {
    return "edgeLabelMarkdownEnabled";
  }
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:edge-added",
      (_canvas, edge) => this.onEdgeChanged(edge)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:edge-changed",
      (_canvas, edge) => this.onEdgeChanged(edge)
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (_canvas, edge) => this.cleanupLabel(edge.labelElement)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-view-unloaded:before",
      (view) => {
  ...
  patchLabelPrototype(labelElement) {
    ...
    Patcher.patchPrototype(this.plugin, labelElement, {
       ...
       */
      setText: Patcher.OverrideExisting((next) => function(text) {
        if (!this.isEditing && that.renderedSources.get(this) === text) return;
        next.call(this, text);
        void that.renderMarkdown(this);
      }),
      destroy: Patcher.OverrideExisting((next) => function(skipSave) {
  ...
  interceptClicks(labelElement) {
    ...
    const interceptor = (event) => {
      ...
      if (!((_a = target == null ? void 0 : target.closest) == null ? void 0 : _a.call(target, INTERACTIVE_SELECTOR))) return;
  ...
  async renderMarkdown(labelElement) {
    var _a, _b, _c;
    if (labelElement.isEditing) return;
    const source = (_a = labelElement.edge.label) != null ? _a : "";
  ...
  unloadRenderComponent(labelElement) {
    var _a;
    (_a = this.renderComponents.get(labelElement)) == null ? void 0 : _a.unload();
    this.renderComponents.delete(labelElement);
  }
  cleanupLabel(labelElement) {
    if (!labelElement) return;
    this.unloadRenderComponent(labelElement);
    this.renderedSources.delete(labelElement);
...
// src/canvas-extensions/edge-arrow-base-canvas-extension.ts
var TRIANGLE_ARROW_LENGTH = 10.4;
var TRIANGLE_ARROW_STYLES = [void 0, null, "triangle", "triangle-outline"];
var DEFAULT_PATH_END_OFFSET = 7;
var MAX_TOTAL_OFFSET_RATIO = 0.45;
function getOutwardSideVector(side) {
  switch (side) {
    case "top":
      return { x: 0, y: -1 };
    case "right":
...
function movedAlong(position, direction, distance) {
  return { x: position.x + direction.x * distance, y: position.y + direction.y * distance };
}
var EdgeArrowBaseCanvasExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    this.pendingZoomRefreshes = /* @__PURE__ */ new Map();
    this.appliedZoomMultipliers = /* @__PURE__ */ new WeakMap();
  }
  isEnabled() {
    return true;
  }
  init() {
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (canvas) => this.scheduleZoomRefresh(canvas)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:settings-changed",
      () => {
  ...
  onEdgePathUpdated(canvas, edge) {
    ...
    if (!edge.bezier || !((_a = edge.path) == null ? void 0 : _a.display)) return;
  ...
  getArrowLength(canvas, edge) {
    var _a, _b;
    const styleAttributes = this.plugin.settings.getSetting("edgesStylingFeatureEnabled") ? (_a = edge.getData()) == null ? void 0 : _a.styleAttributes : void 0;
    if (!TRIANGLE_ARROW_STYLES.includes(styleAttributes == null ? void 0 : styleAttributes.arrow)) return null;
    const arrowSize = Number.parseFloat(
      String((_b = styleAttributes == null ? void 0 : styleAttributes.arrowSize) != null ? _b : this.plugin.settings.getSetting("defaultArrowSize"))
    ) || 1;
  ...
  getZoomMultiplier(canvas) {
    var _a;
    const zoomMultiplier = Number.parseFloat((_a = canvas.wrapperEl) == null ? void 0 : _a.style.getPropertyValue("--zoom-multiplier"));
    return Number.isFinite(zoomMultiplier) && zoomMultiplier > 0 ? zoomMultiplier : Math.sqrt(1 / Math.pow(2, canvas.zoom));
  }
  /**
   ...
   */
  scheduleZoomRefresh(canvas) {
    if (this.pendingZoomRefreshes.has(canvas)) return;
    const waitForViewport = () => {
      var _a;
      if (!((_a = canvas.canvasEl) == null ? void 0 : _a.isConnected)) {
   ...
   */
  rerenderEdges(canvas) {
    var _a;
    for (const edge of canvas.edges.values()) {
      if (edge.initialized && ((_a = edge.path) == null ? void 0 : _a.display.isConnected)) canvas.markDirty(edge);
    }
  }
};
...
var CanvasMetadataExposerExtension = class extends CanvasExtension {
  ...
  updateExposedSettings(canvas) {
    var _a;
    const startNodeId = (_a = canvas.metadata) == null ? void 0 : _a["startNode"];
