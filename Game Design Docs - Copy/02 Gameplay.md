...
- slower bullet speed
- slower move speed
- cursed: stores damage taken and doubles it if you take another hit within n seconds, otherwise sheds it.
- can't use abilities
- can't move
- reduced defence
...
- can't nexus
## Near miss detection
The Inspirations doc mentions this as a wanted feature, so here's what that could actually look like:
- Graze mechanics, like Touhou. A bullet that passes within a couple pixels of your hitbox counts as a "near miss" and the game acknowledges it somehow.
- What the reward would be, idk. Options:
  - A resource meter that fills up and gives you a damage buff or a heal when it's full. Probably too strong, perma-death game and all.
  - Style points / score. Harmless, fun, but kind of pointless in an MMO.
  - Nothing at all, just a sound effect and a little visual pop. Might honestly be enough - grazing feels good on its own.
- Main concern is server load. Tracking every bullet against every player's graze radius at MMO scale sounds like a nightmare. Might have to be client-side only, which makes it cosmetic by default.
- Deferred either way. This is a "the game is fun without it" feature, not a core one.
# End game content
