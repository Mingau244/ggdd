...
### `spawn_enemy`/`despawn_enemy`: manual spawns through the archetype helpers

Both of these (`admin-5`) exist to let an admin place a specific enemy template at a specific point without waiting for [[06 Enemy AI & Bullet Patterns|06]]'s automatic biome-region spawning to roll it. `spawn_enemy` looks the template up, wraps the requested position onto the torus, resolves its chunk index, and delegates to `spawn_enemy_archetype` — the same helper `spawn_from_biome_regions` uses — so a manually spawned enemy gets the identical row set (the whole phase→attack→step behavior tree included) as a naturally spawned one. `despawn_enemy` is the mirror: it looks the `Enemy` row up (erroring if the id doesn't exist) and calls `despawn_enemy_archetype`, deleting the entire behavior tree along with the flat row — the same cleanup `deal_damage_to_enemy`'s kill branch performs. (Both previously bypassed the helpers — `spawn_enemy` double-inserted the `EnemyBehavior` row and `despawn_enemy` leaked the whole tree; those bugs are fixed, and the helpers are now the only spawn/despawn path anywhere in the module.)
...
- **No client UI reaches any reducer this doc covers.** `claim_admin`, `release_admin`, `change_stats`, all five catalog upserts, the four enemy step-def upserts (`upsert_movement_def`/`upsert_single_step_def`/`upsert_repeat_step_def`/`upsert_multi_step_def`), `spawn_enemy`, `despawn_enemy`, `add_chunks`, `clear_chunks`, `generate_world_proc`, `generate_world_manual`, and `toggle_debug` are all only callable via the SpacetimeDB CLI today — there's no in-game admin panel, console, or debug menu of any kind.
